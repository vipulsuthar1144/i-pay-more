...
const page = () => {
  ...
  // const productDetails = dataIPhoneList.find((data) => data.slug === decodeURIComponent(productID as string));
  ...
  const handleSetParams = () => {
    ...
    const params = setParams({
      ...
      ppcsr: productDetails.processor ?? undefined,
      pimg: productDetails?.image,
      st: "SELL",
      dt: productType?.toString().toUpperCase(),
    });
    ...
    <div className=" container m-auto  py-10 space-y-5 ">
      ...
      <div className="bg-white rounded-md  shadow-md p-6 md:p-16 w-full flex flex-col md:flex-row md:items-center  space-x-10 space-y-0 sm:space-y-5 ">
        ...
        <div className=" flex flex-2 flex-col items-start">
          ...
          <p className="text-gray-500 mb-1 text-md">Choose a variant</p>
          ...
          <div className="flex flex-wrap gap-4 justify-start mb-4">
            {productDetails?.storages?.map((storage: any) => (
              <button
                key={storage}
          ...
          <Button
            disabled={
              productDetails?.colors?.length > 0
                ? !selectedColor
                : true || productDetails?.colors?.length > 0
                  ? !selectedStorage
                  : true
            }
