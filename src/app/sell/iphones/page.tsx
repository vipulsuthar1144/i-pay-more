...
const page = () => {
  ...
  const listenerGoToProductDetails = (navigateRoute: string) => {
    navigate.push(`/sell/iphones/${navigateRoute}`);
  };
  return (
    <section className="container m-auto pt-10 space-y-5">
      <h2 className="text-2xl font-semibold text-gray-900 font-heading">Sell your Old Iphone</h2>
      <Breadcrumb />
      <p className="text-lg font-medium text-gray-900 font-heading">Select Model</p>
      <div className=" grid grid-cols-2 gap-4 lg:grid-cols-6">
        {dataIPhoneList?.map((item, index) => (
          <ProductCard
            key={index}
            title={item.model}
            img={item.image}
