...
import DeviceDetails from "@/app/sell/calculate/sections/DeviceDetails";
...
export default function TellUsAbout() {
  ...
  const handleContinue = () => {
    if (currentStep == TOTAL_REPAIR_STEPS) {
      document.getElementById("address-submit-btn")?.click();
    } else if (currentStep == TOTAL_REPAIR_STEPS - 1) {
      ...
      currentStep < TOTAL_REPAIR_STEPS && setCurrentStep((prevStep) => prevStep + 1);
    }
  ...
  const renderBreadcrumb = () => {
    ...
    const BreadcrumbList = [
      ...
      { name: "Repair Services", route: "/calculate" },
    ];
    return (
      <>
  ...
  return (
    <div className={`${root_container} py-10`}>
      ...
      />
      <h2 className="text-2xl font-semibold text-gray-900 font-heading">Repair Your {QueryParams.pmn}</h2>
      {renderBreadcrumb()}
