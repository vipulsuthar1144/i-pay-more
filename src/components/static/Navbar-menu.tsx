...
export const MenuItem = ({
  ...
  return (
    <div
      ...
      //   onClick={() => (active ? setActive(item) : setActive(null))}
      className="relative "
    >
      <motion.p
        transition={{ duration: 0.3 }}
        className="cursor-pointer text-sm md:text-base  text-black hover:underline dark:text-white"
      >
        {item}
      </motion.p>
      {active !== null && (
...
export const Menu = ({
  ...
  return (
    <nav
      ...
      className="relative rounded-full border border-transparent dark:bg-black dark:border-white/[0.2] bg-white shadow-input flex justify-center space-x-6 px-8 py-0 "
    >
      {children}
    </nav>
  );
